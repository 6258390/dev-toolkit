...
import { access, chmod, cp, readdir, readFile, writeFile } from 'fs/promises';
...
cli
  ...
  .action(async (options, ctx) => {
    ...
    await ctx.task('Getting staged files', async () => {
      ...
      stagedFiles.forEach(f => { const m = f.match(/^templates\/([^/]+)\//); if (m) templateNeedsBump.add(m[1]); });
    });

    await ctx.parallel('Bumping packages', async (ctx) => {
      const packagesDir = join(process.cwd(), 'packages');
      if (!await _exists(packagesDir)) return;

      for (const pkgDir of await readdir(packagesDir)) {
        const pkgPath = join(packagesDir, pkgDir);
        const pkgJsonPath = join(pkgPath, 'package.json');
        if (!await _exists(pkgJsonPath) || !stagedFiles.some(f => f.startsWith(`packages/${pkgDir}/`))) continue;
        ...
        await ctx.task(pkgJson.name, async (ctx) => {
          ...
          ctx.log(`📦 Bumped to ${JSON.parse(await readFile(pkgJsonPath, 'utf-8')).version}`);

          const templatesDir = join(process.cwd(), 'templates');
          if (!await _exists(templatesDir)) return;

          for (const tpl of await readdir(templatesDir)) {
            const tplPath = join(templatesDir, tpl);
            const tplJsonPath = join(tplPath, 'package.json');
            if (!await _exists(tplJsonPath)) continue;

            const tplJson = JSON.parse(await readFile(tplJsonPath, 'utf-8'));
            if (tplJson.dependencies?.[pkgJson.name]) {
              execSync(`npm pkg set dependencies.${pkgJson.name}=^${JSON.parse(await readFile(pkgJsonPath, 'utf-8')).version}`, { cwd: tplPath });
    ...
    await ctx.task('Bumping templates', async (ctx) => {
      ...
      for (const tpl of templateNeedsBump) {
        ...
        ctx.log(`📦 ${tplJson.name} → ${JSON.parse(await readFile(tplJsonPath, 'utf-8')).version}`);
      }
    ...
    function _publishFromDir(dirName, renameGitignore = false) {
      return async (ctx) => {
        ...
        for (const subDir of await readdir(dir)) {
          ...
          if (!await _exists(packageJsonPath)) continue;
          ...
          await ctx.task(packageJson.name, async (ctx) => {
            if (renameGitignore && await _exists(join(path, '.gitignore'))) execSync('mv .gitignore gitignore', { cwd: path });
...
cli
  ...
  .action(async (options, ctx) => {
    ...
    await ctx.task('Copying template files', async (ctx) => {
      ...
      await Promise.all(items.map(item => cp(join(templateDir, item), join(targetDir, item === 'gitignore' ? '.gitignore' : item), { recursive: true })));
...
cli
  ...
  .action(async (options, ctx) => {
    await ctx.task('Creating pre-commit hook', async () => {
      await writeFile('.git/hooks/pre-commit', `#!/usr/bin/env sh
